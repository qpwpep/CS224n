...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        random_len = random.randint(4, int(self.block_size * 7/8))
        truncated_doc = document[:random_len]

        quarter_len = len(truncated_doc) // 4
        min_masked_len = max(1, quarter_len // 2)
        max_masked_len = min(len(truncated_doc) - 1, quarter_len * 3 // 2)
        masked_len = random.randint(min_masked_len, max_masked_len)

        masked_start = random.randint(0, len(truncated_doc) - masked_len)
