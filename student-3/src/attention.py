...
def precompute_rotary_emb(dim, max_positions):
    ...
    dim_index = torch.arange(half_dim).unsqueeze(0)

    theta = 1.0 / (10000 ** (2 * dim_index / dim))
...
def apply_rotary_emb(x, rope_cache):
    ...
    rope = rope_cache[:T].to(x.device)

    x_          = x.view(*x.shape[:-1], D // 2, 2)
    x_complex   = torch.view_as_complex(x_)

    rope_cplx   = torch.view_as_complex(rope)
    rope_cplx   = rope_cplx.unsqueeze(0).unsqueeze(0)

    rotated_cplx = x_complex * rope_cplx
    rotated = torch.view_as_real(rotated_cplx)
    rotated_x = rotated.view_as(x)
    ### END YOUR CODE ###
    return rotated_x
